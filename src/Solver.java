...
public class Solver {
    ...
    */
    ...
    /** Solve the system. The initial guess and the result are 
	in the same array.
	@param x the solution.
     */
    public void solve(double [] x) throws InterruptedException {
	if (es.number()==0) return;
	...
	while (list.get(0).getM()>1.0) {
	    ...
	    // 	    System.out.println(""+list.get(j).getIndex(0)+" "+list.get(j).getIndex(1)+" "+list.get(j).getM()+" "+list.get(j).getF(0)+" "+list.get(j).getF(1));
	    // 	System.exit(0);
	    // }
	...
	public Point(double []x) {
	    ...
	    try {
		es.equations(x,f);
	    } catch (Exception ex) { f[0]=10000.0; }
	    m=rms=0.0;
	    for (int j=0;j<es.number();j++) {
	...
	public double getIndex(int n) { return x[n]; }
	public double getF(int n) { return f[n]; }
	public int compareTo(Point p) {
	    if (getM()<p.getM()) return -1;
	    if (getM()>p.getM()) return 1;
	    return 0;
	}
