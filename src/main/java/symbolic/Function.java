...
public class Function extends Expression {
	...
	private static Expression inverse(Expression e) {
		return new Power(e, -1);
	}
	...
	public Expression differentiate(String x) {
		...
		switch (type) {
		case SIN:
			d = new Function(COS, arg);
			break;
		case COS:
			d = new Negative(new Function(SIN, arg));
			break;
		case TAN:
			d = new Power(new Function(COS, arg), -2);
			break;
		case ASIN:
			d = inverse(new Function(SQRT, new Sum(new Constant(1.), new Negative(new Power(arg, 2)))));
			break;
		case ACOS:
			d = inverse(new Function(SQRT, new Sum(new Constant(1.), new Negative(new Power(arg, 2)))));
			break;
		case ATAN:
			d = inverse(new Sum(new Constant(1.), new Power(arg, 2)));
			break;
		case SINH:
			d = new Function(COSH, arg);
			break;
		case COSH:
			d = new Function(SINH, arg);
			break;
		case TANH:
			d = new Power(new Function(COSH, arg), -2);
			break;
		case ASINH:
			d = inverse(new Function(SQRT, new Sum(new Constant(1.), new Power(arg, 2))));
			break;
		case ACOSH:
			d = inverse(new Function(SQRT, new Sum(new Constant(-1.), new Power(arg, 2))));
			break;
		case ATANH:
			...
			break;
		case EXP:
			d = new Function(EXP, arg);
			break;
		case LOG:
			d = inverse(arg);
			break;
		case SQRT:
			d = inverse(new Product(new Constant(2.), new Function(SQRT, arg)));
			break;
		}
		if (d == null)
	...
	public double evaluate(String[] vs, double[] xs) {
		...
		switch (type) {
		case SIN:
			return Math.sin(sarg);
		case COS:
			return Math.cos(sarg);
		case TAN:
			return Math.tan(sarg);
		case ASIN:
			return Math.asin(sarg);
		case ACOS:
			return Math.acos(sarg);
		case ATAN:
			return Math.atan(sarg);
		case SINH:
			return Math.sinh(sarg);
		case COSH:
			return Math.cosh(sarg);
		case TANH:
			return Math.tanh(sarg);
		case ASINH:
			return Double.NaN; // Math.asinh(sarg);
		case ACOSH:
			return Double.NaN; // Math.acosh(sarg);
		case ATANH:
			return Double.NaN; // Math.atanh(sarg);
		case EXP:
			return Math.exp(sarg);
		case LOG:
			return Math.log(sarg);
		case SQRT:
			return Math.sqrt(sarg);
		}
