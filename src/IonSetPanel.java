...
public class IonSetPanel extends JPanel {
    ...
    /** Creates the IonSetPanel, a graphical element for handling 
	an IonSet.
    */
    public IonSetPanel() {
	...
	public RemoveActionListener(IonPanel ri) { rri=ri; }
	public void actionPerformed(ActionEvent e) {
	    removeIonPanel(rri);
	}
    }
    ...
    */
	...
	if (!f) {
	    int j=list.size()-1;
	    if (list.get(j).isX.isSelected()) j--;
	...
	if (!f) {
	    int j=list.size()-1;
	    if (list.get(j).isLast.isSelected()) j--;
	...
	for (int j=0;j<list.size();j++) {
	    ...
	    if (list.get(j).isX.isSelected()) iX=i;
	    else 
		if (list.get(j).isLast.isSelected()) iLast=i;
		else is.add(i);
	}
	if (!nonzero0) 
	...
	if ((iX.getCL()-iX.getC0())/1000.0==0.0)
	    throw new Exception("The concentrations c0 and cL must be different for the ion set as \"X\", called "+iX);
	is.add(iX);
	is.add(iLast);
	return is;
    }
    ...
    */
	...
	int jX=-1, jLast=-1;
	for (int j=0;j<list.size();j++) {
	    if (list.get(j).isX.isSelected()) jX=j;
	    if (list.get(j).isLast.isSelected()) jLast=j;
	}
	int j=0;
	for (int k=0;k<is.size()-2;k++) {
	    while ((j==jX)||(j==jLast)) j++;
